...
# 4 checkers where even blocks have temperature 100
# and rest have temperature 0
u[:int(SIZE_X/2), :int(SIZE_Y/2), 0] = 100
u[int(SIZE_X/2):, int(SIZE_Y/2):, 0] = 100
...
def laplacian(
  ...
  # we want to make it so du\dn=0, where n is the normal
  # along the boundary so if one of the neighbours goes
  # out of bounds, we set its value to be the same
  # as the current cell u[x, y, t]

  u_center = u[x, y, t]

  # u_{i-1,j}^n
  # left edge has du\dx = 0, so value 
  # "beyond" border should match u_center
  u_left = u_center if x == 0 else u[x - 1, y, t]
  ...
  # u_{i,j-1}^n
  # bottom edge has du\dy = 0, so value 
  # "beyond" border should match u_center
  u_bottom = u_center if y == SIZE_Y - 1 else u[x, y + 1, t]
